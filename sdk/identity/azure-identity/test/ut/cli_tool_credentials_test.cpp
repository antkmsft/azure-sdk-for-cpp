...
namespace {
enum class CliCredentialKind
{
  AzureCli,
};

class AzureCliCredentialTestHelper final {
private:
  ~AzureCliCredentialTestHelper() = default;
  static int g_localTimeToUtcDiffSeconds;
  static int GetLocalTimeToUtcDiffSeconds() { return g_localTimeToUtcDiffSeconds; }

public:
  static void SetLocalTimeToUtcDiffSeconds(int diff) { g_localTimeToUtcDiffSeconds = diff; }
  static AccessToken ParseAzureCliToken(const std::string& cliCommandOutput)
  {
    return CliToolCredentialImpl::ParseAzureCliToken(
        cliCommandOutput, GetLocalTimeToUtcDiffSeconds);
  }
};

int AzureCliCredentialTestHelper::g_localTimeToUtcDiffSeconds = 0;

const std::map<CliCredentialKind, std::string> CliCredentialKindToCredentialName
    = {{CliCredentialKind::AzureCli, "AzureCliCredential"}};

const std::map<
    CliCredentialKind,
    std::function<std::string(const std::string& scopes, const std::string& tenantId)>>
    CliCredentialKindToGetCliCommandFunction
    = {{CliCredentialKind::AzureCli, CliToolCredentialImpl::GetAzureCliCommand}};

const std::map<CliCredentialKind, std::function<AccessToken(const std::string& cliCommandOutput)>>
    CliCredentialKindToParseTokenFunction
    = {{CliCredentialKind::AzureCli, AzureCliCredentialTestHelper::ParseAzureCliToken}};
...
TEST_P(CliToolCredentialImpl, Success)
#else
TEST_P(CliToolCredentialImpl, NotAvailable)
#endif
{
  ...
  CliToolCredentialImpl const cliTokenCredentialImpl(
      "CliTokenCredentialImpl",
      "",
      std::chrono::seconds(10),
      {},
      [&Token](const std::string& scopes, const std::string& tenantId) {
        return EchoCommand(Token);
      },
      CliToolCredentialImpl::ParseAzureCliToken);
