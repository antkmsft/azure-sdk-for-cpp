...
void CliToolCredentialImpl::ThrowIfNotSafeCmdLineInput(
  ...
  for (auto const c : input)
  {
    ...
    if (!StringExtensions::IsAlphaNumeric(c))
    {
      throw AuthenticationException(
          m_credentialName + ": Unsafe command line input found in " + description + ": " + input);
    }
  }
}

CliToolCredentialImpl::CliToolCredentialImpl(
    ...
    std::function<std::string(const std::string& scopes, const std::string& tenantId)>
        getCliCommand,
    std::function<Core::Credentials::AccessToken(const std::string& cliCommandOutput)> parseToken)
    : m_getCliCommand(std::move(getCliCommand)), m_parseToken(std::move(parseToken)),
      m_additionallyAllowedTenants(std::move(additionallyAllowedTenants)),
      m_credentialName(std::move(credentialName)), m_tenantId(std::move(tenantId)),
...
AccessToken CliToolCredentialImpl::GetToken(
    ...
    Context const& context) const
{
  ...
  auto const command = m_getCliCommand(scopes, tenantId);
  ...
  return m_tokenCache.GetToken(scopes, tenantId, tokenRequestContext.MinimumExpiration, [&]() {
    try
    {
      auto const cliToolOutput = RunShellCommand(command, m_cliProcessTimeout, context);

      try
      {
        return m_parseToken(cliToolOutput);
      }
      catch (json::exception const&)
      {
        ...
        throw std::runtime_error(cliToolOutput.substr(0, 250));
      }
    }
    catch (std::exception const& e)
    {
      auto const errorMsg = m_credentialName + " didn't get the token: \"" + e.what() + '\"';
