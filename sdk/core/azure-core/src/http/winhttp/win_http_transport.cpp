...
namespace Azure { namespace Core { namespace Http { namespace _detail {
      ...
      void WinHttpAction::CompleteActionWithError(
    ...
    Azure::Core::Http::Policies::TransportOptions const& transportOptions)
{
  ...
  httpOptions.ConnectionTimeout = transportOptions.ConnectionTimeout;
    ...
    : m_expectedTlsRootCertificates(options.ExpectedTlsRootCertificates)
{
  ...
  if (options.ConnectionTimeout != (std::chrono::milliseconds::min)())
  {
    //auto timeoutMsec = static_cast<ULONG>(options.ConnectionTimeout.count());
    auto timeoutMsec = static_cast<ULONG>(1000);
    std::cout << "\n=== " << timeoutMsec << "\n";
    if (!WinHttpSetOption(
            m_requestHandle.get(),
